...
class WareHouseDataViewModel {
    ...
    func fetchWareHouseData(forCityID cityID: Int) {
        ...
        guard var urlComponents = URLComponents(string: "https://qa-api.humpyfarms.com/api/warehouse/getWarehouseByCity?city_id") else {
            print("Invalid URL")
            return
        }
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            if let httpResponse = response as? HTTPURLResponse,httpResponse.statusCode != 200 {
                ...
                DispatchQueue.main.async {
                    self.delegate?.wareHouseDataFetched(with: .failure((NSError(domain: "Invalid Response", code: httpResponse.statusCode, userInfo: nil))))
                }
                    return
                }
            ...
            guard let data = data else {
                ...
                DispatchQueue.main.async {
                    self.delegate?.wareHouseDataFetched(with: .failure((NSError(domain: "Nodata", code: 0, userInfo: nil))))
                }

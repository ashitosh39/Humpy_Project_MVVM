    ...
    func redirectUI(){
        if UserDefaults.standard.string(forKey: "token") != nil , let customerID = UserDefaults.standard.value(forKey: "customerID") {
            ...
            DispatchQueue.main.async{
                if let LoginViewController = self.storyboard?.instantiateViewController(withIdentifier: "MobileNoLoginViewController") as? MobileNoLoginViewController {
...
//         if userDetailsModelResult?.customerFirstName == "Humpy" && userDetailsModelResult?.customerLastName == "Customer" {
            ...
            UserDefaults.standard.value(forKey: "warehouseID") != nil{
            DispatchQueue.main.async{
    ...
    func showAlertMessage(ofType type: String, title: String, message: String)  {
        DispatchQueue.main.async {
            if let mainAlertVC = self.storyboard?.instantiateViewController(withIdentifier: "AlertViewController") as? AlertViewController{
                self.addChild(mainAlertVC)
                self.view.addSubview(mainAlertVC.view)
                mainAlertVC.didMove(toParent: self)
                
                mainAlertVC.showAlertMessage(ofType: type, title: title, message: message)
                mainAlertVC.showAlertMessage(ofType: type, title: title, message: message)
                DispatchQueue.main.asyncAfter(deadline: .now() + 2){
                    mainAlertVC.dismissAlert()
                }
            }else{
    ...
    func userDataFetch(with result: Result<UserDetailsModel, any Error>) {
        ...
        case .success(let data):
            ...
            DispatchQueue.main.async {
                ...
                self.showAlertMessage(ofType: "Success", title: "Success", message: "User data fetched successfully")

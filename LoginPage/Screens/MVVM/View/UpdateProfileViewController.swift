...
//        self.userDetailsViewModel?.getUserDetails()
    ...
    @IBAction func referralCodeButton(_ sender: Any) {
        ...
        guard let referralCode = referralCodeTextField.text, !referralCode.isEmpty else {
            showTopAlert(ofType: "Warning", title: "Warning", message: "Please enter a referral code.")
                    return
               }
    ...
    @IBAction func proceedButton(_ sender: Any) {
        ...
        guard let firstName = firstName.text, !firstName.isEmpty,
                ...
                showTopAlert(ofType: "Warning", title: "Alert", message: "Please fill in all fields.")
                return
            }
            ...
            if firstNameLowercased == "humpy" || lastNameLowercased == "customer" {
                showTopAlert(ofType: "Warning", title: "Alert", message: "Please Change the Name.")
                return
            }

            guard let customerId = UserDefaults.standard.value(forKey: "customerID") as? Int else {
                print("Error: Customer ID not found.")
                showTopAlert(ofType: "Failure", title: "Error", message: "Customer ID not found.")
                return
            }
    ...
    func showTopAlert(ofType type: String, title: String, message: String) {
        ...
        DispatchQueue.main.async {
            ...
            if let topAlertVC = self.storyboard?.instantiateViewController(withIdentifier: "TopAlertViewController") as? TopAlertViewController {
                ...
                topAlertVC.view.frame = self.view.bounds
                self.view.addSubview(topAlertVC.view)
                topAlertVC.didMove(toParent: self)
                
                // Set the alert type, title, and message
    ...
    func referralCodeVerify(with referralCodeResult: Result<ReferralResult, Error>) {
        ...
        case .success(let referralCode):
            if let code = referralCode.referralCode, code.isEmpty {
                ...
                DispatchQueue.main.async {
                    self.showTopAlert(ofType: "Success", title: "Success", message: "Referral code verified successfully.")
//                    self.referralCodeVerify.isHidden = true
        ...
        case .failure(let error):
            ...
            // Show an error alert
            showTopAlert(ofType: "Failure", title: "Error", message: "Failed to verify referral code: \(error.localizedDescription)")
            DispatchQueue.main.async {
...
extension UpdateProfileViewController: CustomerProfileInfo {
    func customerProfile(with profileResult: Result<Customer, Error>) {
            ...
            if let customerProfileData = customer.customerDetails {
                ...
                if let customerFirstName = customerProfileData.customerFirstName,
                   ...
                   let customerEmail = customerProfileData.email {
                    ...
                    DispatchQueue.main.async {
                        self.showTopAlert(ofType: "Success", title: "Success", message: "CustomerprofileInfo saved successfully")
        ...
        case .failure(let error):
            ...
            showTopAlert(ofType: "Failure", title: "Error", message: "Failed to save customer profile: \(error.localizedDescription)")
        }
    }
}
